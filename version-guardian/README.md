...
**אין להשוות `display_version` ל־`source_version_number`.** לדוגמה, גרסת מוצר 82 ו־Sites source version 86 יכולות להיות תקינות לחלוטין יחד.

## מצב חי שנמדד בעת עדכון ה־MVP

- גרסת מוצר מוצגת: `82`.
- ChatGPT Sites source version: `86`.
- projection revision: `45`.
...
- `guard-release.mjs` — שער שחרור אוטומטי.
...
לא נחשב תקלה:
- `display_version=82` יחד עם `source_version_number=86`.
...
הבדיקות כוללות:
- הפרדה תקינה בין גרסת מוצר 82 ל־Sites source 86.
...
שער השחרור נבדק גם ידנית: runtime תקין החזיר exit 0; display version 81 מול Manifest 82 נחסם עם `RELEASE BLOCKED` ו־exit 1.

## צעד הבא

כאשר מקור ה־ChatGPT Site הקנוני זמין לעריכה מתוך סביבת הכלים, לשתול את שלושת קבצי הרכיב ולהוסיף את `version-manifest.json` לתהליך הפריסה. לאחר מכן כל גרסה חדשה חייבת לעדכן Manifest כחלק מהשחרור, לא ידנית לאחר הפריסה.
