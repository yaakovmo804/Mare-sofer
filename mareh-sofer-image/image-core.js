  ...
  function processImageData(input, width, height, options) {
    ...
    for (let y = 0; y < height; y += 1) {
      for (let x = 0; x < width; x += 1) {
        ...
        const bgR = clamp(source[p] + backgroundDetail * (textureGain - 1) + brightness + warm * 22, 0, 255);
        const bgG = clamp(source[p + 1] + backgroundDetail * (textureGain - 1) + brightness + warm * 12, 0, 255);
        const bgB = clamp(source[p + 2] + backgroundDetail * (textureGain - 1) + brightness - warm * 14, 0, 255);
