(() => {
  ...
  const autoStraightenBtn = byId('autoStraightenBtn');
  const rotateLeftBtn = byId('rotateLeftBtn');
  const rotateRightBtn = byId('rotateRightBtn');
  const beforeCanvas = byId('beforeCanvas');
  const afterCanvas = byId('afterCanvas');
  const previewStage = byId('previewStage');
  const emptyState = byId('emptyState');
  const afterClip = byId('afterClip');
  const splitLine = byId('splitLine');
  const splitInput = byId('splitInput');
  const strengthInput = byId('strengthInput');
  const strengthOutput = byId('strengthOutput');
  const geometryMetric = byId('geometryMetric');
  const maskToggle = byId('maskToggle');
  const statusText = byId('statusText');
  const resolutionText = byId('resolutionText');
  const angleStatus = byId('angleStatus');
  const installHint = byId('installHint');
  const dismissInstallHint = byId('dismissInstallHint');
  ...
  function scheduleRender() {
    ...
    requestAnimationFrame(() => {
      ...
      if (token !== state.renderToken) return;
      if (maskToggle.checked) drawMask(result.mask);
      else afterContext.putImageData(new ImageData(result.data, state.width, state.height), 0, 0);
  ...
  function showInstallHint() {
    ...
    const dismissed = localStorage.getItem('marehSoferInstallHintDismissed') === '1';
    installHint.hidden = standalone || dismissed;
  }
  ...
  dismissInstallHint.addEventListener('click', () => {
    localStorage.setItem('marehSoferInstallHintDismissed', '1');
